...
func (p *Processor) HandleUEContextTransferRequest(c *gin.Context,
	...
	if problemDetails != nil {
		sbi.UEContextTransferCounter.With(prometheus.Labels{"StatusCode": strconv.Itoa(int(problemDetails.Status))}).Inc()
		c.JSON(int(problemDetails.Status), problemDetails)
	} else {
		sbi.UEContextTransferCounter.With(prometheus.Labels{"StatusCode": strconv.Itoa(http.StatusOK)}).Inc()
